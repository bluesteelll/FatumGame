...
## Компоненты магазинов

Магазины -- это entity предметов с дополнительными компонентами отслеживания патронов. Определены в `FlecsItemComponents.h`.

### FMagazineStatic (Prefab)

| Поле | Тип | Описание |
|------|-----|----------|
| `CaliberId` | `uint8` | Индекс из CaliberRegistry (0xFF = невалидный) |
| `Capacity` | `int32` | Максимум патронов (по умолчанию 30) |
| `WeightPerRound` | `float` | Вес одного патрона (кг) |
| `ReloadSpeedModifier` | `float` | Множитель скорости перезарядки |
| `AcceptedAmmoTypes[8]` | `UFlecsAmmoTypeDefinition*[]` | Принимаемые типы патронов |
| `AcceptedAmmoTypeCount` | `int32` | Количество принимаемых типов патронов |

### FMagazineInstance (Экземпляр)

Стек LIFO для патронов. `AmmoSlots[AmmoCount-1]` = следующий патрон для выстрела.

| Поле | Тип | Описание |
|------|-----|----------|
| `AmmoSlots[60]` | `uint8[]` | Индексы типов патронов (в `FMagazineStatic::AcceptedAmmoTypes`) |
| `AmmoCount` | `int32` | Текущее количество патронов в магазине |

Методы: `Pop()` (возвращает индекс типа патрона или -1), `Push(uint8)` (возвращает false если полон), `IsEmpty()`, `IsFull(Capacity)`.

### FAmmoTypeRef (Экземпляр)

На россыпных патронах в инвентаре. Хранит индекс типа патрона для быстрого поиска при поштучной перезарядке.

| Поле | Тип | Описание |
|------|-----|----------|
| `AmmoTypeIndex` | `int32` | Индекс в `FMagazineStatic::AcceptedAmmoTypes` (-1 = не разрешён) |

### Тег магазина

| Тег | Назначение |
|-----|-----------|
| `FTagMagazine` | Сущность является магазином |

---

## Устройства быстрой загрузки

Обоймы и спидлоадеры -- это entity предметов, которые пакетно вставляют патроны в оружие с поштучной перезарядкой. Определены в `FlecsItemComponents.h`.

### EQuickLoadDeviceType

| Значение | Описание |
|----------|----------|
| `StripperClip` | Вставляет патроны во внутренний магазин |
| `Speedloader` | Загружает все патроны сразу (требуется пустой магазин) |

### FQuickLoadStatic (Prefab)

| Поле | Тип | Описание |
|------|-----|----------|
| `DeviceType` | `EQuickLoadDeviceType` | StripperClip или Speedloader |
| `RoundsHeld` | `int32` | Количество заряжаемых патронов за использование |
| `CaliberId` | `uint8` | Индекс калибра из CaliberRegistry |
| `AmmoTypeDefinition` | `UFlecsAmmoTypeDefinition*` | Тип патронов, предзаряженных в устройство |
| `InsertTime` | `float` | Длительность пакетной вставки (секунды) |
| `bRequiresEmptyMagazine` | `bool` | Магазин должен быть пуст для использования |

### Битовые константы

Используются в `FWeaponStatic::AcceptedDeviceTypes`:

| Константа | Значение | Описание |
|-----------|----------|----------|
| `QUICKLOAD_BIT_STRIPPERCLIP` | `1 << 0` | Оружие принимает обоймы |
| `QUICKLOAD_BIT_SPEEDLOADER` | `1 << 1` | Оружие принимает спидлоадеры |

### Тег быстрой загрузки

| Тег | Назначение |
|-----|-----------|
| `FTagQuickLoadDevice` | Сущность является устройством быстрой загрузки |
