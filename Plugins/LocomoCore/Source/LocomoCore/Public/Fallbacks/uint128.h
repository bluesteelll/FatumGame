...
public:
  ...
  //add explicit suppression. we know what we did.
#pragma warning( suppress : 4146 )
  constexpr UnsignedInt128(int n) : low_{static_cast<std::uint64_t>(n)}, high_{-static_cast<std::uint64_t>(n < 0)} {}

#pragma warning( suppress : 4146 )
  constexpr UnsignedInt128(long n): low_{static_cast<std::uint64_t>(n)}, high_{-static_cast<std::uint64_t>(n < 0)} {}
  
#pragma warning( suppress : 4146 )
  constexpr UnsignedInt128(long long n): low_{static_cast<std::uint64_t>(n)}, high_{-static_cast<std::uint64_t>(n < 0)} {}
...
private:
  constexpr int LeadingZeroes() const {
    if (high_ == 0) {
      return 64 + _lzcnt_u64(low_);
    } else {
      return _lzcnt_u64(high_);
    }
  }
