...
INLINE unsigned popcount(__m64 arg) noexcept {
    return LIBPOPCNT_H::popcnt64(*reinterpret_cast<uint64_t*>( &arg)); // i think this is right?
}
    INLINE unsigned popcount(uint64_t arg) noexcept {
    return LIBPOPCNT_H::popcnt64(arg); // i think this is right?
}
