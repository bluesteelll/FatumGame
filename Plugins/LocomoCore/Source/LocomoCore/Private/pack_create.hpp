...
// Boost licensed. Not yet adapted to THST, UE, & Jolt
#pragma once

//This is a bulk-load approach to RTree creation that's extremely fast.
//this removes the main problem with Rtrees entirely.

//we'll use this algorithm if we start to see perf issues, combined with a zorder presort.
...
/*
 *everything here will need replaced.
...
constexpr int THE_MEAT_IS_BELOW = true;
...
struct nth_element_and_half_boxes
{
                             ...
                             Box & left, Box & right, std::size_t dim_index)
    {
        if (I == dim_index)
        {
            ...
            /* per the docs: 
            Description
            The convert algorithm converts one geometry, e.g. a BOX, to another geometry, e.g. a RING.
            This only works if it is possible and applicable. If the point-order is different, or the
            closure is different between two geometry types, it will be converted correctly by explicitly
            reversing the points or closing or opening the polygon rings.

            This... is taking a box to a box?
            #1#
