...
namespace FlecsSaveTypeIds
{
	...
	// ═══════════════════════════════════════════════════════════════
	// 0x05xx — DESTRUCTIBLE / DOOR / PROJECTILE / PENETRATION (Phase 5)
	// ═══════════════════════════════════════════════════════════════

	/** FDoorInstance — state machine + auto-close timer + target position + open direction.
	 *  ConstraintKey NOT serialized; regenerated by DoorSystem first tick post-load. */
	inline constexpr uint16 kTypeId_DoorInstance           = 0x0500;

	/** FDoorTriggerLink — TargetDoorKey is the door entity's BarrageKey (uint64).
	 *  Saved as the door's SaveIndex (REMAP); decoder resolves new entity → new BarrageKey. */
	inline constexpr uint16 kTypeId_DoorTriggerLink        = 0x0501;

	/** FDebrisInstance — lifetime + auto-destroy + pool index + free-mass + pending impulse. */
	inline constexpr uint16 kTypeId_DebrisInstance         = 0x0502;

	/** FProjectileInstance — lifetime + bounce count + grace + fuse + OwnerEntityId (REMAP) + SpawnPosition. */
	inline constexpr uint16 kTypeId_ProjectileInstance     = 0x0503;

	/** FPenetrationInstance — remaining budget + count + damage multiplier + LastPenetratedTargetId (REMAP). */
	inline constexpr uint16 kTypeId_PenetrationInstance    = 0x0504;

	/** FSurfaceIntegrity — 148-byte raw block (Integrity grid + projection metadata). */
	inline constexpr uint16 kTypeId_SurfaceIntegrity       = 0x0505;
	...
	// ═══════════════════════════════════════════════════════════════
	// 0x09xx — SPAWNING DOMAIN (Phase 5)
	// ═══════════════════════════════════════════════════════════════

	/** FSpawnerProvenance — level-path + actor-name of the AFlecsEntitySpawner that produced
	 *  the entity. Used at load time by UFlecsSaveSubsystem to mark matching spawners with
	 *  bSavedEntityOverridesMe so they skip their BeginPlay spawn. */
	inline constexpr uint16 kTypeId_SpawnerProvenance             = 0x0900;
	...
	// ═══════════════════════════════════════════════════════════════
	// 0x10xx — CORE TAGS — Phase 5 additions
	// ═══════════════════════════════════════════════════════════════

	/** FTagPlayerCharacter — identifies the unique 1P player entity for actor re-bind. */
	inline constexpr uint16 kTypeId_TagPlayerCharacter     = 0x1001;
	...
	// ═══════════════════════════════════════════════════════════════
	// 0x15xx — DESTRUCTIBLE / DOOR / PROJECTILE TAGS (Phase 5)
	// ═══════════════════════════════════════════════════════════════

	/** FTagDoor — entity is a door (hinged or sliding). */
	inline constexpr uint16 kTypeId_TagDoor                = 0x1500;

	/** FTagDoorTrigger — entity is a trigger that can unlock/toggle a door. */
	inline constexpr uint16 kTypeId_TagDoorTrigger         = 0x1501;

	/** FTagDestructible — entity can be destroyed by damage. */
	inline constexpr uint16 kTypeId_TagDestructible        = 0x1502;

	/** FTagProjectile — entity is a projectile in flight. */
	inline constexpr uint16 kTypeId_TagProjectile          = 0x1503;

	/** FTagCharacter — entity is a character/player (any character, not just the player). */
	inline constexpr uint16 kTypeId_TagCharacter           = 0x1504;
}
