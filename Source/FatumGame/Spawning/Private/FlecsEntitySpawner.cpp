	...
	const FEntitySpawnRequest& Request)
{
	...
	FlecsSubsystem->EnqueueCommand([FlecsSubsystem, Data]()
	{
		...
		if (PS)
		{
			...
			ProjInst.SpawnPosition = Data.SpawnLocation;
		...
		// GetOrCreateEntityPrefab (Step 13b). The per-instance setup (slot
		// containers, FCraftingSlots/FFuelSlot/FCraftingStationInstance, UI
		// shared state registration) is extracted into FlecsMultiblockRuntime
		// so Phase 2 multiblock bonding can reuse the exact same code path.
		// ─────────────────────────────────────────────────────────
		if (Data.EntityDefinition && Data.EntityDefinition->CraftingStationProfile)
		{
			FlecsMultiblockRuntime::SetupStationInstance(
				Entity, Data.EntityDefinition->CraftingStationProfile);
		}
