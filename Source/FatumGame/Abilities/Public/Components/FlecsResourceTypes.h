...
struct FResourcePools
{
	...
	/** Create from a UFlecsResourcePoolProfile Data Asset (game thread only).
	 *  Returns empty pools (PoolCount=0) if Profile is nullptr. */
	static FResourcePools FromProfile(const class UFlecsResourcePoolProfile* Profile);
};
