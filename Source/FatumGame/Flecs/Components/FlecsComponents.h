...
// ═══════════════════════════════════════════════════════════════
//
// COLLISION PAIR ARCHITECTURE:
// ────────────────────────────────────────────────────────────────
// Each Barrage collision creates a COLLISION PAIR ENTITY with:
//   - FCollisionPair: contact data (entities, point, normal)
//   - Classification tags: determine which systems process it
//
// Flow:
//   1. OnBarrageContact() → creates collision pair entity
//   2. Collision systems query pairs by tags and process
//   3. CollisionCleanupSystem destroys all pairs at end of tick
//
// Classification tags (zero-size, for query filtering):
//   - FTagCollisionDamage: projectile/damage source hit health target
//   - FTagCollisionPickup: character touched pickupable item
//   - FTagCollisionBounce: projectile bounced (any surface)
//   - FTagCollisionDestructible: something hit destructible
//
// Systems process ONLY their tagged pairs - O(1) archetype lookup.
// ═══════════════════════════════════════════════════════════════
 ...
 * Systems query this + classification tags to process specific collision types.
 */
struct FCollisionPair
{
	/** First entity in collision (may be invalid if not Flecs-tracked) */
	uint64 EntityId1 = 0;

	/** Second entity in collision (may be invalid if not Flecs-tracked) */
	uint64 EntityId2 = 0;
	...
	/** World-space contact point (from BarrageContactEvent::PointIfAny) */
	...
	/** Get the projectile entity ID (0 if neither is projectile) */
	...
	/** Get the target (non-projectile) entity ID */
...
/** Collision involves damage: projectile/FDamageSource hit FHealthData target */
struct FTagCollisionDamage {};

/** Collision is item pickup: FTagCharacter touched FTagPickupable + FTagItem */
struct FTagCollisionPickup {};

/** Collision is projectile bounce: projectile hit any surface */
struct FTagCollisionBounce {};

/** Collision hit destructible: something hit FTagDestructible */
struct FTagCollisionDestructible {};

/** Collision between two characters (for future: knockback, etc) */
struct FTagCollisionCharacter {};

/** Marker: this collision pair has been processed (prevents double-processing) */
struct FTagCollisionProcessed {};

// ═══════════════════════════════════════════════════════════════
// LEGACY COLLISION COMPONENT (deprecated, kept for compatibility)
// ═══════════════════════════════════════════════════════════════

/**
 * @deprecated Use FCollisionPair + classification tags instead.
